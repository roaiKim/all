...
function updateRootState(moduleNameInArg) {
    ...
    const replacedRootStateFileContent =
        rootStateFileContent.substring(0, lastStateImportIndex + 6) +
        `\nimport { State as ${moduleStateName} } from "module/${moduleNameInArg}/type";` +
        rootStateFileContent.substring(lastStateImportIndex + 6, lastStateDeclarationIndex) +
...
function generate() {
    ...
    try {
        ...
        if (splitModuleNames.length > 1 && !available1stLevelModuleNames.includes(splitModuleNames[0]))
            throw new Error(`模块 [${moduleNameInArg.split("/")[0]}] 不在可用空间(${available1stLevelModuleNames.join("/")})下`);
        ...
        if (!nameRegex.test(splitModuleNames[0]) || !nameRegex.test(splitModuleNames[1]))
            throw new Error(`模块名称格式错误: ${moduleNameInArg}。 只允许小写, 或用中划线连接; E.g: rosen、rosen-one`);
