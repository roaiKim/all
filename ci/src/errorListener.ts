    ...
    onError(error: Exception) {
        if (error instanceof APIException) {
            if (error.statusCode === 401 || error.statusCode === 403) {
                ...
                createErrorMessage(process.env.NODE_ENV === "production" ? "发生网络错误，请稍后重试" : error.message);
            }
        } else if (error instanceof NetworkConnectionException) {
            createErrorMessage("网络连接超时，请稍后重试");
        } else {
            const errorMessage = process.env.NODE_ENV === "production" ? "发生错误，请稍后重试" : error.message;
