...
import { useIframeCommunicator } from "./postmessage";
import "./index.less";
...
function Home(props: HomeProps) {
    ...
    const { sendMessage, subscribe } = useIframeCommunicator();
    ...
    // const { subscribe, sendMessage } = usePostMessage();

    const listenerMessage = (event) => {
        console.log("接收到了父元素的信息", event);
    };

    useEffect(() => {
        const unSubscribe = subscribe("PARENT_HELLO", listenerMessage);
        return unSubscribe;
    }, []);
            ...
            <PageTable.header title="运单管理" modalState={pageModalState}>
                <Button
                    ...
                    onClick={() => {
                        ...
                        setTimes((prev) => ({ ...prev, times: prev.times + 1 }));
                        setTimes((prev) => ({ ...prev, times: prev.times + 1 }));
                        sendMessage("CHILDREN_HELLO", { text: "欢迎, 来啦老弟" });
                    }}
                >
