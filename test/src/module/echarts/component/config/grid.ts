...
const GridConfig: EchartsTreeState[] = [
    ...
    },
    {
        ...
        description: ["直角坐标系（grid）组件的宽度。默认自适应。", "width 的值可以是像 20 这样的具体像素值，可以是像 '20%' 这样相对于容器宽度的百分比。"],
        options: [],
        propTypes: ["string", "number"],
        descendant: [],
        isLeaf: true,
    },
    {
        ...
        description: ["直角坐标系（grid）组件的高度。默认自适应。", "height 的值可以是像 20 这样的具体像素值，可以是像 '20%' 这样相对于容器高度的百分比。"],
        options: [],
        propTypes: ["string", "number"],
        descendant: [],
        isLeaf: true,
