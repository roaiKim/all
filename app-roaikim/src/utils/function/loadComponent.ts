...
export function calcCacheModule() {
    ...
    const deafaultTabs = deafaultTabPaths
        ...
        .filter(Boolean);
    return { deafaultTabs, modulesCache, nameToPath, pathToName };
}

const { deafaultTabs, modulesCache, nameToPath, pathToName } = calcCacheModule();
